...
set_kubernetes_cluster() {
  &>/dev/null kubectl config set-cluster "${KUBERNETES_CLUSTER_NAME}" \
...
set_credentials() {
  &>/dev/null kubectl config set-credentials "${KUBERNETES_USER_NAME}" \
    --client-certificate="${USER_CERT}" \
    --client-key="${USER_CERT_PRIVATE_KEY}"
}

set_cluster_context() {
  &>/dev/null kubectl config set-context "$KUBERNTES_CLUSTER_CONTEXT" \
    --cluster="$KUBERNETES_CLUSTER_NAME" \
    --user="$KUBERNETES_USER_NAME"
}

enable_context() {
  &>/dev/null kubectl config use-context "$KUBERNTES_CLUSTER_CONTEXT"
}
