...
KUBERNETES_PUBLIC_PORT="${KUBERNETES_PUBLIC_PORT?Please provide the port to access Kubernetes on.}"
...
PRIVATE_KEY_PATH_VARIABLE="${PRIVATE_KEY_PATH_VARIABLE:-temporary_ssh_key}"
SSH_TEMP_KEY_NAME="${SSH_TEMP_KEY_FORMAT:-/tmp/kubernetes_cluster_ssh_key}"
...
_is_key_value_pair() {
  ...
  echo "$key_value_pair_under_test" | grep -Eq "^[a-zA-Z0-9_]{1,}=\".*\"$"
}
...
_create_s3_ssh_key_bucket_if_not_present() {
  ...
  then
    aws s3 mb "s3://$SSH_KEY_S3_BUCKET_NAME" &>/dev/null
    return_code=$?
  fi
  return "$return_code"
}
...
_create_and_download_ssh_key_from_s3() {
  ...
  if ! aws s3 ls "s3://$SSH_KEY_S3_BUCKET_NAME/$SSH_KEY_S3_KEY_PATH/$ENVIRONMENT_NAME" &>/dev/null
  then
    >&2 echo "WARN: SSH key not found; creating."
    ssh-keygen -t rsa -f "$path_to_download_to" -b 2048 -N "" &>/dev/null;
    aws s3 cp "$path_to_download_to" "s3://$SSH_KEY_S3_BUCKET_NAME/$SSH_KEY_S3_KEY_PATH/$ENVIRONMENT_NAME" &>/dev/null;
  else
    aws s3 cp "s3://$SSH_KEY_S3_BUCKET_NAME/$SSH_KEY_S3_KEY_PATH/$ENVIRONMENT_NAME" "$path_to_download_to" &>/dev/null
  fi
  set +e
}

delete_preexisting_ssh_keys() {
  rm -rf "${SSH_TEMP_KEY_NAME}"*
}

create_or_retrieve_kubernetes_nodes_key_pair() {
  tfvars_path="${1?Please provide the path to the variables file.}"
  temporary_ssh_keypair_path="$SSH_TEMP_KEY_NAME"
  ...
  chmod 600 "$temporary_ssh_keypair_path"
  ...
  echo "${PRIVATE_KEY_PATH_VARIABLE}=\"${temporary_ssh_keypair_path}\"" >> "$tfvars_path"
