#!/usr/bin/env bash
export AWS_REGION = "${AWS_REGION?Please provide an AWS region to operate in.}"
export AWS_ACCESS_KEY_ID = "${AWS_ACCESS_KEY_ID?Please provide an access key.}"
export AWS_SECRET_ACCESS_KEY = "${AWS_SECRET_ACCESS_KEY?Please provide a secret key.}"
export TERRAFORM_STATE_S3_BUCKET = "${TERRAFORM_BACKEND_S3_BUCKET?Please provide the S3 bucket into which state  will be stored.}"
export TERRAFORM_STATE_S3_KEY = "${TERRAFORM_BACKEND_S3_KEY?Please provide the S3 key within the bucket into which state  will be stored.}"
export KUBERNETES_INFRASTRUCTURE_SOURCE_PATH = "$(git rev-parse --show-toplevel)/kubernetes/control_plane/aws"
export TERRAFORM_BACKEND_PATH = "${KUBERNETES_INFRASTRUCTURE_SOURCE_PATH}/backend.tf"
export TERRAFORM_PROVIDER_PATH = "${KUBERNETES_INFRASTRUCTURE_SOURCE_PATH}/provider.tf"
export AUTOGENERATED_TERRAFORM_TFVARS_PATH = "${KUBERNETES_INFRASTRUCTURE_SOURCE_PATH}/terraform.tfvars"
export TEMPLATED_TERRAFORM_TFVARS_PATH = "${AUTOGENERATED_TERRAFORM_TFVARS_PATH}.template"
