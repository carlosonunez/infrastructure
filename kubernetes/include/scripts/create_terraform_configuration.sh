...
create_or_retrieve_kubernetes_nodes_key_pair() {
  tfvars_path="${1?Please provide the path to the variables file.}"
  temporary_ssh_keypair_path="/tmp/temporary_key"
  if [ ! -f "$temporary_ssh_keypair_path" ]
  then
    if ! ssh-keygen -t rsa -f "$temporary_ssh_keypair_path" -b 2048 -N ''
    then
      >&2 echo "ERROR: Unable to create SSH keys."
      return 1
    fi
  fi
